...
| แถว | ปุ่ม |
|---|---|
| ทำ | 🔢 กรอกเลขที่อยากจอง · 📋 เลขที่เฝ้าอยู่ · 🏆 จองได้แล้ว · 🧹 ลบประวัติแชตเก่า · 🌐 เข้าสู่เว็บไซต์ |
...
## 🏆 จองได้แล้ว → เตือนก่อนหมดเขตจดทะเบียน

จองเลขได้ในเว็บขนส่งแล้ว **กดปุ่ม 🏆 บนแผงแล้วใส่เลข** — bot จะเตือนในห้องก่อนหมดเขตจดทะเบียน
(ค่าเริ่มต้น 7 วัน และ 1 วันก่อน · ตั้งได้ที่ `reminders.daysBeforeRegisterDeadline`)
**เลยกำหนดแล้วเลขที่จองได้จะหลุด** ต้องไปจองใหม่

- หมวดอักษรกับวันหมดเขต **ไม่ต้องกรอก** ถ้าเลขอยู่ในตารางสัปดาห์นี้ (bot หาให้) · ถ้าจองไว้นานแล้วค่อยพิมพ์เอง
- จดทะเบียนเรียบร้อยแล้ว กด 🏆 อีกครั้งแล้วใส่เลขในช่องล่างสุดเพื่อหยุดเตือน
- bot **ไม่ได้เช็คกับระบบขนส่ง** ว่าคุณจองอะไรได้ (ADR-0001) — ข้อมูลนี้คุณเป็นคนบอก · รายละเอียด: [ADR-0008](docs/adr/0008-register-deadline-from-user-declared-plates.md)

## 🔨 เลขที่ขนส่งกันไว้ประมูล — จองออนไลน์ไม่ได้

ขนส่งกันเลขสวย **301 หมายเลขต่อหนึ่งหมวดอักษร** ไว้ประมูลที่ tabienrod.com (เลขสี่ตัวเหมือน · เลขตอง · เลขตัวเดียว ·
เลขหลักพัน · เลขเรียง · เลขคู่ `6644` `6446` `6464` ฯลฯ) เลขกลุ่มนี้ไม่เข้าระบบจองออนไลน์ตั้งแต่แรก
[`auction-rules.json`](auction-rules.json) เก็บรายการนี้ไว้ (สร้างจากตารางแนบท้ายประกาศกรมการขนส่งทางบกด้วย `npm run gen:auction`
— มี `sources` กำกับ และสคริปต์จะล้มถ้าอ่านได้ไม่ครบ 301)

bot จะ **ไม่แจ้งให้ไปกดจอง** เลขพวกนี้ แต่แสดงไว้ในช่อง 🔨 ของการ์ด 🎯 และปุ่ม 📋 พร้อมชื่อกลุ่ม
เจอเลขอื่นที่กดแล้วจองไม่ได้ ใส่ในช่อง 🔨 ของปุ่ม 🔢 ได้เลย (`wishlist.auction`) · รายละเอียด: [ADR-0006](docs/adr/0006-auction-numbers-filtered-by-published-rules.md)

> ⚠️ ถ้า wishlist ของคุณเป็น "เลขตอง" หรือ "เลขคู่สลับ" ล้วน ๆ จะพบว่าแทบไม่เหลือเลขให้จองเลย —
> นั่นคือความจริงของระบบ ไม่ใช่ bot พัง · ลองเงื่อนไขที่ไม่ชนกลุ่มประมูล เช่น `^\d(\d)\1\1$` (5111) หรือผลรวมที่ชอบ
...
npm run gen:auction สร้าง auction-rules.json ใหม่จากประกาศฯ (รันเมื่อขนส่งแก้ประกาศ)
...
src/
  ...
  won.ts              เลขที่ผู้ใช้จองได้แล้ว + วันหมดเขตจดทะเบียน (เก็บใน meta.won · ADR-0008)
...
- ตารางที่รองรับคือของ **สำนักงานขนส่งกรุงเทพฯ พื้นที่ 5** (ระบบกลางของขนส่ง) · เลขที่ต้องประมูลแยกไว้ในช่อง 🔨 ไม่ใช่เลขที่จองได้
