...
- สถานะ: ยอมรับ · 2026-09-20 · **แก้ไข 2026-09-21: ฐานข้อมูลย้ายจาก Supabase ไป Neon** (ดูหมายเหตุท้ายไฟล์)
...
## หมายเหตุ 2026-09-21 · ย้าย Postgres จาก Supabase ไป Neon

ผู้ใช้ชี้ว่า Supabase แผนฟรีมีได้ **2 โปรเจกต์** และอยากเก็บ slot ไว้ให้โปรเจกต์ใหญ่ · ถามว่าใช้ MongoDB แทนได้ไหม

ข้อเท็จจริง: โค้ดไม่ได้ใช้อะไรที่เป็น Supabase เลย (ไม่มี auth ไม่มี Data API) ใช้แค่ Postgres ผ่าน connection string
จึงมี 2 ทาง: **Neon** (Postgres ฟรี 10 โปรเจกต์ · แก้ค่า `DATABASE_URL` ค่าเดียว · โค้ด 0 บรรทัด) หรือ **MongoDB Atlas**
(เขียน `src/db/` ใหม่ ~200 บรรทัด · unique index + transaction ต้องทำเอง) → เลือก **Neon** เพราะได้สิ่งที่ต้องการ
(คืน slot) โดยไม่รื้อของที่เพิ่งทดสอบผ่าน · ย้ายด้วย `npm run db:copy` จำนวนแถวตรงกันทุกตาราง

ผลที่ตามมา: ทุกที่ที่เขียน "Supabase" ในเอกสารให้อ่านว่า "Postgres (ตอนนี้ Neon)" · Neon หลับเมื่อไม่มี connection
และตื่นใน ~0.5 วิ — กระทบแค่ cron ช้าขึ้นครึ่งวิ · MongoDB ยังเป็นทางเลือกได้ผ่าน `Store` interface ถ้ามีเหตุผลใหม่
