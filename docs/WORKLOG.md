...
**เจอตอนยิงจริงครั้งแรก (11:40):** ผู้ใช้กด 🧹 ตอน bot เพิ่งโพสต์การ์ด → ลบข้อความรัว ๆ ทำให้คำสั่งลบแผงเก่าโดน **429**
แล้วเงียบไปเพราะ `.catch()` → แผงค้างสองใบในห้อง · แก้ที่ `discordRest`: เจอ 429 ให้รอ `retry_after` แล้วลองใหม่ครั้งเดียว
เทสที่เขียนคู่กันยังเผยว่า `redactPath` กันเฉพาะ app id ที่เป็นตัวเลข → เปลี่ยนเป็น `[^/]+` ปิดทั้ง app id และ token
